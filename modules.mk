...
OBJ_DIR     = sim_obj
SRC_DIR     = hardware
D_TESTS_DIR = modules-tests
D_EXTRA_DIR = debug-tests-extra

#--------------------------------------------------------------------
# Sources
#--------------------------------------------------------------------
objs_basename = $(basename $(notdir $(wildcard $(SRC_DIR)/*.v)))
objs_no_test  = $(patsubst %_tb.cpp, %, $(notdir $(filter-out $(objs_basename), $(wildcard $(D_TESTS_DIR)/*_tb.cpp))))
...
objs_extra	  = $(wildcard $(D_EXTRA_DIR)/*.cpp)


#--------------------------------------------------------------------
# Build Rules
#--------------------------------------------------------------------
VERILATOR_CFLAGS = -CFLAGS "-std=c++11 -O3 -I../$(D_EXTRA_DIR)"
VERILATOR_OPTS = -Wall --Mdir $(OBJ_DIR) -y $(SRC_DIR) -Wno-lint --trace --exe 

INCS = -I /mingw$(shell getconf LONG_BIT)/include/libelf -I../$(D_EXTRA_DIR)
...
$(objs_verilate): %.verilate: $(SRC_DIR)/%.v
	...
	@verilator $(VERILATOR_CFLAGS) $(VERILATOR_OPTS) --cc $< --top-module $(basename $(notdir $<)) $(patsubst %.verilate, $(D_TESTS_DIR)/%_tb.cpp, $@) $(objs_extra)
